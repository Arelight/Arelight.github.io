...
<html lang="en" dir="ltr">
	<head>
		...
		<title>M&K autohaven | Товар</title>
	</head>
	<body>
		<div class="all">
			<div class="nav">
				<a href="index.php"><i class="fas fa-angle-double-left"></i></a><h1>M&K autohaven</h1>
			</div>
			<div class="container">
				<div class="results">
					<h3>Корзина <span class="cost"><i class="fas fa-donate"></i> <?php echo $total; ?> </span> <a href="checkout.php?user_id=<?php echo $user_id; ?>" class="checkout"> Общая </a></h3>
					<div class="product">
                        <?php
                        if($cartss->num_rows == 0){
                            echo '<img class="empty-cart" src="assets/Display_images/empty_cart.png">';
                            echo '<h1 class="empty-h1"> Пустая корзина </h1>';
                        }
                            ...
                            if($row['product_id'] != -1){
                                echo '<div class="item">
                                    ...
                                    <div class="item-info">
                                        ...
                                        <p style="font-size: 10px; margin-top:20px; color: gray;"></p>
                                        <form class="" action="product.php?id='.$row['product_id'].'" method="post">
                   					 	    <button type="submit" class="order-btn"> Обзор</button>
                   					    </form>
                                        <form action="cart.php?cnl_id='.$row['id'].'" method="post">
                                            <button class="cnl-btn" type="submit" name="cnl-submit">Отмена</button>
                                        </form>
                                    </div>
                                </div>';
                            }
